...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void up(Long spuId) {
        ...
        List<MySkuEsModel.Attrs> attrsList = baseAttrs.stream().filter(item -> {
            return idSet.contains(item.getAttrId());
        }).map(item -> {
            MySkuEsModel.Attrs attrs = new MySkuEsModel.Attrs();
        ...
        List<MySkuEsModel> collect = skuInfoEntities.stream().map(sku -> {
            //组装需要的数据
            MySkuEsModel esModel = new MySkuEsModel();
